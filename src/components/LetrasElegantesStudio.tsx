...
export const LetrasElegantesStudio: React.FC<LetrasElegantesStudioProps> = ({
  ...
  return (
    <section className="bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-pink-500/10 rounded-2xl border border-indigo-200/80 p-6 sm:p-8 shadow-xs">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800">
                💎 Caligrafía & Lujo
              </span>
            </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 transition-all active:scale-95"
        >
      ...
      <div className="bg-white rounded-xl border border-indigo-200/90 p-4 sm:p-5 mb-6 shadow-xs">
        ...
        <div className="flex gap-2 mb-4">
          <input
            ...
            maxLength={35}
            className="flex-1 px-3.5 py-2.5 rounded-xl border border-slate-300 bg-slate-50 text-slate-900 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
          {elegantStylesForText(customText || 'Elegante').map((styled, idx) => (
            <div
              key={idx}
              className="flex items-center justify-between p-2.5 rounded-lg border border-slate-200 bg-slate-50/70 hover:border-indigo-300 transition-all group"
            >
              ...
              <button
                ...
                className="p-1.5 rounded-md bg-white text-slate-600 hover:bg-indigo-600 hover:text-white transition-colors shrink-0 shadow-2xs"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
        {elegantSignatures.map((sig, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between p-3.5 rounded-xl border border-slate-200/90 bg-white shadow-2xs hover:border-indigo-400 hover:shadow-xs transition-all group"
          >
            <div>
            ...
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white font-bold text-xs transition-all active:scale-95"
            >
              {copiedItem === sig.text ? (
                <>
                  <Check className="w-3.5 h-3.5 text-emerald-600" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {bioTemplates.map((template, idx) => (
          <div
            key={idx}
            className="flex flex-col justify-between p-4 rounded-xl border border-slate-200 bg-white shadow-2xs hover:border-indigo-300 transition-all"
          >
            <pre className="text-xs font-serif text-slate-800 whitespace-pre-wrap leading-relaxed mb-3">
              {template}
            </pre>
            <button
              type="button"
              onClick={() => handleCopy(template)}
              className="w-full inline-flex items-center justify-center gap-1.5 py-2 rounded-lg bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white font-bold text-xs transition-all active:scale-95"
            >
              {copiedItem === template ? (
                <>
                  <Check className="w-3.5 h-3.5 text-emerald-600" />
